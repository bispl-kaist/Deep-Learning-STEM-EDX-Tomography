...
class Model(object):
    def __init__(self, sess, opt):
        ...
        self.bn = opt.bn
        self.lambda_random = opt.lambda_random
	...
	# input data
        self.all_input_data_conv = tf.placeholder(tf.float32, shape=[self.batchSize*self.no_random, self.nc, self.imageSize, self.imageSize])
        self.split_input_data_conv = tf.split(self.all_input_data_conv, len(self.devices))

	# mask data
        self.all_mask_data_conv = tf.placeholder(tf.float32, shape=[self.batchSize, self.nc, self.imageSize, self.imageSize])
        self.split_mask_data_conv = tf.split(self.all_mask_data_conv, len(self.devices))
	...
	# cost
        ...
        for device_index, (device, label_data_conv, input_data_conv, mask_data_conv) in enumerate(zip(self.devices, self.split_label_data_conv, self.split_input_data_conv, self.split_mask_data_conv)):
            with tf.device(device):
                ...
                self.all_fake_data 	= self.attention(self.batchSize*self.no_random, noise=self.input_data, nc=self.nc, isize=self.imageSize, name=gen_name, is_avg=self.avg)
	...
	# Train loop 
        iteration = -1
        train_idx = loadData.get_data_idx(root=self.dataroot)
        test_idx = loadData.get_data_idx(root=self.dataroot_val)
        
        _fixed_test_inputs, _fixed_test_labels, _fixed_test_mask = batchLoader.getBatch(test_idx, 0)
        ...
        while iteration < self.epochs:	
            ...
            for i in range(TotalBATCHNum):
                ...
                for i_gen in range(gen_iters):
                    nbatch += 1
                    _input, _label, _mask = batchLoader.getBatch(train_idx, nbatch)

                    _gen_cost, _fake_data, _ = self.sess.run([self.gen_cost, self.all_fake_data, self.gen_train_op], 
                        feed_dict={self.all_label_data_conv:_label, self.all_input_data_conv:_input, self.all_mask_data_conv:_mask, self.lrG:self._lrG})
	    ...
	    # validataion
            ...
            for dev_iter in range(dev_TotalBATCHNum):
                ...
                for dev_i_gen in range(gen_iters):	   
                    dev_nbatch += 1
                    _dev_input, _dev_label, _dev_mask = batchLoader.getBatch(test_idx, dev_nbatch) 
                    
                    _dev_gen_cost, _dev_fake_data = self.sess.run([self.gen_cost, self.all_fake_data],
                               feed_dict={self.all_label_data_conv: _dev_label, self.all_input_data_conv:_dev_input, self.all_mask_data_conv:_dev_mask})
            ...
            if iteration % self.nsave == self.nsave-1:      
                _fixed_fake_data = self.sess.run(self.all_fake_data, feed_dict={self.all_label_data_conv: _fixed_test_labels, self.all_input_data_conv: _fixed_test_inputs, self.all_mask_data_conv:_fixed_test_mask})
    ...
    def calculate_cost(self):
        if self.proj:
            self.all_label_cost = tf.multiply(self.all_label_data_conv, self.all_mask_data_conv)
        else:
            self.all_label_cost = self.all_label_data_conv

        if self.regcost=='l2':
            gen_cost 		= tf.reduce_mean((self.all_label_cost - self.all_fake_data)**2)
        elif self.regcost=='l1':
            gen_cost 	= tf.reduce_sum(tf.abs(self.all_label_cost - self.all_fake_data))
