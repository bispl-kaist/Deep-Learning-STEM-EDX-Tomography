        ...
        for root, _, fnames in sorted(os.walk(d)):
            for fname in sorted(fnames):	    
                if is_image_file(fname):
                    ...
                    path_input_concat_tmp   = path_label.replace(ddir, os.path.join(dir, 'Input_concat'))
                    for idn in range(no_random):
                        if 'si' in path_input_tmp:
                            path_input      = path_input_tmp.replace('si', str(idn+1)+'_si')
                            path_concat     = path_input_concat_tmp.replace('si', str(idn+1)+'_si')
                            item            = (path_input, path_concat, path_label)
                            images.append(item)
