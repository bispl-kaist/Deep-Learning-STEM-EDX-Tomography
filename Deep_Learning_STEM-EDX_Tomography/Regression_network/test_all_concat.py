...
class Model_test_concat(object):
	...
	# input data
        ...
        self.all_concat_input_data_conv = tf.placeholder(tf.float32, shape=[None, self.nc*2, None, None])
        self.split_concat_input_data_conv = tf.split(self.all_concat_input_data_conv, len(self.devices))
	
        self.all_fake_data = []

        for device_index, (device, input_data_conv, concat_input_data_conv) in enumerate(zip(self.devices, self.split_input_data_conv, self.split_concat_input_data_conv)):
            with tf.device(device):
                self.input_data	= input_data_conv
                self.concat_all_data = tf.concat([self.input_data, concat_input_data_conv], 1)
                ...
                self.fake_data 	= self.generator(None, noise=self.concat_all_data, nc=self.nc, bn=self.bn, is_concat=True,
	...
	# Train loop
        ...
        while data_iteration < testDataNum-1:
            ...
            self.path_concat_img = test_idx[data_iteration][1]
            overallConcat = self.pil_loader(self.path_concat_img)
            overallConcat = np.asarray(overallConcat, dtype=np.float32)

            self.path_label = test_idx[data_iteration][2]
            overallLabel = self.pil_loader(self.path_label)
            overallLabel = np.asarray(overallLabel, dtype=np.float32)

            maxVal = np.amax(overallLabel)
            overallInput = overallImg/(maxVal+1e-15)
            overallConcat = overallConcat/(maxVal+1e-15)
            ...
            _concat_input = np.transpose(overallConcat, (2, 0, 1))
            _concat_input = np.expand_dims(_concat_input, axis=0)
            
            _fake_data = self.sess.run(self.all_fake_data, feed_dict={self.all_input_data_conv:_input, self.all_concat_input_data_conv:_concat_input})
            ...
            scipy.misc.toimage(_rec, cmin=0, cmax=1).save(os.path.join('{0}/{1}.png'.format(self.experiment, data_iteration)))	
