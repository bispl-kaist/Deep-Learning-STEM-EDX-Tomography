...
import tflib.plot
import math
from datasets import loadData_concat as loadData
from datasets.getBatch_concat import Batch
import random
import utils.make_image as makeImage
from scipy import signal
import scipy.io as sio

class Model_concat(object):
	...
	# input data
        ...
        # concat input data
        self.all_concat_input_data_conv     = tf.placeholder(tf.float32, shape=[self.batchSize, self.nc*2, self.imageSize, self.imageSize])
        self.split_concat_input_data_conv   = tf.split(self.all_concat_input_data_conv, len(self.devices))

	# cost
        self.gen_costs = []
        self.all_fake_data = []
        
        for device_index, (device, label_data_conv, input_data_conv, concat_input_data_conv) in enumerate(zip(self.devices, self.split_label_data_conv, self.split_input_data_conv, self.split_concat_input_data_conv)):

            with tf.device(device):
                self.label_data = label_data_conv
                self.input_data = input_data_conv
                self.concat_input_data  = concat_input_data_conv

                self.concat_all = tf.concat([self.input_data, self.concat_input_data], 1)
                ...
                self.fake_data  = self.generator(self.batchSize/len(self.devices), noise=self.concat_all, nc=self.nc, bn=self.bn, is_concat=True,
					isize=self.imageSize, OUTPUT_DIM=self.outputDim, initialization=self.initialization, filtSize=self.filtSize, name=gen_name)
		
                if self.residual:
                    self.fake_data = tf.add(tf.nn.relu(self.fake_data), input_data_conv)
	...
	# Train loop 
        ...
        _fixed_test_inputs, _fixed_test_concat_inputs, _fixed_test_labels   = batchLoader.getBatch(test_idx, 0)
        ...
        while iteration < self.epochs:	
            ...
            for i in range(TotalBATCHNum):
                ...
                for i_gen in range(gen_iters):
                    nbatch += 1
                    _input, _concat_input, _label = batchLoader.getBatch(train_idx, nbatch)
                    
                    _gen_cost, _fake_data, _ = self.sess.run([self.gen_cost, self.all_fake_data, self.gen_train_op], 
                            feed_dict={self.all_label_data_conv:_label, self.all_input_data_conv:_input, self.all_concat_input_data_conv:_concat_input, self.lrG:self._lrG})
	    ...
	    # validataion
            ...
            for dev_iter in range(dev_TotalBATCHNum):
                ...
                for dev_i_gen in range(gen_iters):	   
                    dev_nbatch += 1
                    _dev_input, _dev_concat_input, _dev_label = batchLoader.getBatch(test_idx, dev_nbatch) 
                    
                    _dev_gen_cost, _dev_fake_data = self.sess.run([self.gen_cost, self.all_fake_data],
                            feed_dict={self.all_label_data_conv: _dev_label, self.all_input_data_conv:_dev_input, self.all_concat_input_data_conv:_dev_concat_input})
            ...
            if iteration % self.nsave == self.nsave-1:
                ...
                _fixed_fake_data = self.sess.run(self.all_fake_data, feed_dict={self.all_label_data_conv: _fixed_test_labels, self.all_input_data_conv: _fixed_test_inputs, self.all_concat_input_data_conv:_fixed_test_concat_inputs})
