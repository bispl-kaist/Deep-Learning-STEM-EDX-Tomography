...
%SQD2-2
load('12_spherical_projection_particle_2_rot_70_s_se_zn_zn(se)_zn(s)_inter_linear_step_1_thr_0.003.mat')
...
xlabel('Phi(\Phi)'); ylabel('Theta(\theta)'); title('SQD2-2 Se map'); colorbar;
...
xlabel('Phi(\Phi)'); ylabel('Theta(\theta)'); title('SQD2-2 S map'); colorbar;
hold on
for k = 1:length(B)
boundary = B{k};
...
xlabel('Phi(\Phi)'); ylabel('Theta(\theta)'); title('SQD2-2 Zn map'); colorbar;

subplot(3,2,2); histogram(thickness_se_1(:),[0:45]); title('sQD2-2(Se)'); ylabel('Count'); xlabel('Thickness (Pixel)');
subplot(3,2,4); histogram(thickness_s_1(:),[0:45]); title('sQD2-2(S)'); ylabel('Count'); xlabel('Thickness (Pixel)');
subplot(3,2,6); histogram(thickness_zn_1(:),[0:45]); title('sQD2-2(Zn)'); ylabel('Count'); xlabel('Thickness (Pixel)');
...
formatSpec = "The Statistical Values (Se, S, Zn, Non Sulfur Area) of SQD2-2 are  %2.1f\x00B1%1.1f, %2.1f\x00B1%1.1f, %2.1f\x00B1%1.1f, %2.1f.";
