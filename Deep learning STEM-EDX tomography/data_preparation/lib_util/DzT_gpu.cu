...
///////////////////////////////////////////////////////////////////////////
            ...
            void    RunDzT(float *pdDst, float *pdSrc, int *pnSizeSrc);
__global__  void    DzT(float *pdDst, float *pdSrc, int nY, int nX, int nZ);
...
void RunDzT(float *pdDst, float *pdSrc, int *pnSizeSrc)
{
    ...
    DzT<<<nGridNum, nBlockNum>>>(gpupdDst, gpupdSrc, nY, nX, nZ);
...
__global__  void    DzT(float *pdDst, float *pdSrc, int nY, int nX, int nZ)
{
    ...
    int     nIdxZpost   = nIdxZ + 1 < nZ ? nIdxZ + 1: 0;
    
    int     nIdx        = nY*nX*nIdxZ       + nY*nIdxX  + nIdxY;
    int     nIdxpost	= nY*nX*nIdxZpost	+ nY*nIdxX	+ nIdxY;
    
    pdDst[nIdxpost]  	= - (pdSrc[nIdxpost] - pdSrc[nIdx]);
