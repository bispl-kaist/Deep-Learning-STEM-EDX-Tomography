...
class Model_test(object):
	...
	# input data
        self.all_input_data_conv = tf.placeholder(tf.float32, shape=[None, self.nc, self.imageSize, self.imageSize])
    ...
    def Test(self, opt):
        ...
        while data_iteration < testDataNum-1:
            ...
            _, ImgW, _ = overallInput.shape

            self.start = timeit.default_timer()
	  
            _input = np.transpose(overallInput, (2, 0, 1))
            _input = np.expand_dims(_input, axis=1)

            _fake_data = self.sess.run(self.all_fake_data,
					feed_dict={self.all_input_data_conv:_input})
